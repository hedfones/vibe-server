...
with Session(engine) as session:
    ...
    location_product_link_count = len(session.exec(select(LocationProductLink)).all())
    associate_product_link_count = len(session.exec(select(AssociateProductLink)).all())
